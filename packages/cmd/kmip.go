...
func startKmipServer(cmd *cobra.Command, args []string) {
	...
	if strategy == util.AuthStrategy.UNIVERSAL_AUTH {
		identityClientId, err = util.GetCmdFlagOrEnv(cmd, "identity-client-id", []string{util.KMS_UNIVERSAL_AUTH_CLIENT_ID_NAME})

		if err != nil {
			util.HandleError(err, "Unable to parse identity client ID")
		}

		identityClientSecret, err = util.GetCmdFlagOrEnv(cmd, "identity-client-secret", []string{util.KMS_UNIVERSAL_AUTH_CLIENT_SECRET_NAME})
		if err != nil {
